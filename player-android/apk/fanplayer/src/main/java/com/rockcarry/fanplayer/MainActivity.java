...
public class MainActivity extends Activity {
    ...
    private FanMediaNativePlayer mPlayer    = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.ACTION_VIEW.equals(action)) {
            ...
            mPlayer = new FanMediaNativePlayer(mURL, mHandler, PLAYER_INIT_PARAMS);
        } else {
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_UPDATE_PROGRESS: {
                    ...
                    int progress = mPlayer != null ? (int)mPlayer.getParam(FanMediaNativePlayer.PARAM_MEDIA_POSITION) : 0;
                    if (!mIsLive) {
            ...
            case MSG_UDPATE_VIEW_SIZE: {
                ...
                break;
            case FanMediaNativePlayer.MSG_OPEN_DONE: {
                    if (mPlayer != null) {
                        mPlayer .setDisplaySurface(mVideoSurface);
                        mVideo  .setVisibility(View.INVISIBLE);
                        mHandler.sendEmptyMessage(MSG_UDPATE_VIEW_SIZE);
                        mSeek.setMax((int)mPlayer.getParam(FanMediaNativePlayer.PARAM_MEDIA_DURATION));
                        testPlayerPlay(true);
                    }
                }
                break;
            case FanMediaNativePlayer.MSG_OPEN_FAILED: {
                    String str = String.format(getString(R.string.open_video_failed), mURL);
                    Toast.makeText(MainActivity.this, str, Toast.LENGTH_LONG).show();
                }
                break;
            case FanMediaNativePlayer.MSG_PLAY_COMPLETED: {
                    if (!mIsLive) finish();
                }
                break;
            case FanMediaNativePlayer.MSG_VIDEO_RESIZED: {
                    mVideo.setVisibility(View.INVISIBLE);
                    mHandler.sendEmptyMessage(MSG_UDPATE_VIEW_SIZE);
                }
                break;
